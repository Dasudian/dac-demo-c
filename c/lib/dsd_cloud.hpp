...
#ifdef DSD_CLOUD_H
#warning "Only need to include dsd_cloud.hpp (C++), not dsd_cloud.h (C)"
#endif
#ifndef DSD_CLOUD_HPP
#define DSD_CLOUD_HPP
...
typedef struct dsd_cloud_instance_t dsd_cloud_instance_t;
...
class API
{
    ...
    public:
        ...
        int publish(char const * const name,
                        void const * const request,
                        uint32_t const request_size) const;

        inline int publish(std::string const & name,
                               void const * const request,
                               uint32_t const request_size) const
        {
            return publish(name.c_str(),
                               request,
                               request_size);
        }
    ...
    private:
        dsd_cloud_instance_t * const m_api;
    ...
    public:
        ...
        //            << errinfo_stack(dsd_cloud::API::backtrace()));
        //
