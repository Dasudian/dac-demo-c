...
#endif
		pres_s=a.list_all(string("/db/dsdb/ocean/my_db"));
		log<<"get list"<<*pres_s<<"\n"<<endl;
		delete pres_s;
...
{ 
      log.open("/tmp/cxxlist.txt", ios::app);
	   clog_init_path(SDK_LOG,"/tmp/cxxsdklist.log");
	   clog_set_level(SDK_LOG, CLOG_INFO);
      API m=API(0);
      API::function_object_cxx *cb= new cxx_callback(m);
      m.subscribe(string("cxxlist/post"),cb);
      m.poll();
      return 0;
